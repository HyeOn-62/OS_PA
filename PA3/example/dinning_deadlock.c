...
pickup(int phid)
{
	...
	pthread_mutex_lock(&(chopstick[left].mutex)) ; noise() ;
	pthread_mutex_lock(&(chopstick[right].mutex)) ; noise() ;
}
...
putdown(int phid)
{
	...
	pthread_mutex_unlock(&(chopstick[right].mutex)) ; noise() ;
	pthread_mutex_unlock(&(chopstick[left].mutex)) ; noise() ;
}
